...
class TextProperties:
    """Text properties extracted from a Fusion Text+ generator.

    Stored for human-readable diffs (e.g. "changed text from X to Y").
    The Fusion comp file (.comp) is the source of truth for full restore.
    """
    styled_text: str = ""
    font: str = ""
    size: float = 0.0
...
class VideoItem:
    ...
    item_type: str = "media"
    generator_name: str = ""
    fusion_comp_file: Optional[str] = None
    ...
    def from_dict(cls, d: dict) -> "VideoItem":
        ...
        if "text_properties" in d:
            text_props = TextProperties.from_dict(d["text_properties"])

        item_type = d.get("item_type", "media")
        generator_name = d.get("generator_name", "")

        # Backward compat: placeholder PNGs saved by the old serializer
        # (before it detected them as generators) should still be treated
        # as titles so the deserializer restores them via Fusion comp.
        name = d.get("name", "")
        if item_type == "media" and "placeholder_" in name and ".png" in name:
            item_type = "title"
            generator_name = generator_name or "Text+"
