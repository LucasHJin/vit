...
Step-by-step walkthrough to test vit end-to-end with DaVinci Resolve Free.
...
|---|---|
| **Vit - Save Version** | Snapshot the timeline and commit |
| **Vit - New Branch** | Create a branch (e.g. "color-grade") |
| **Vit - Switch Branch** | Checkout a branch and restore the timeline |
| **Vit - Merge Branch** | Merge another branch into yours |
| **Vit - Push** | Push your branch to GitHub for collaborators |
| **Vit - Pull & Restore** | Pull collaborator's changes and restore timeline |
| **Vit - Status** | See current branch, changes, and recent history |
...
```bash
cd /Users/lucasjin/Documents/Hackathons/vit
...
```
  Linked: Vit - Save Version.py → .../resolve_plugin/vit_commit.py
  Linked: Vit - New Branch.py → .../resolve_plugin/vit_branch.py
  Linked: Vit - Merge Branch.py → .../resolve_plugin/vit_merge.py
  Linked: Vit - Switch Branch.py → .../resolve_plugin/vit_restore.py
  Linked: Vit - Status.py → .../resolve_plugin/vit_status.py
  Linked: Vit - Push.py → .../resolve_plugin/vit_push.py
  Linked: Vit - Pull & Restore.py → .../resolve_plugin/vit_pull.py

  Installed 7 scripts to Resolve.
  Restart Resolve, then find them under Workspace > Scripts.
```

### 1c. Initialize a vit project

```bash
mkdir ~/Desktop/vit-test-project
vit init ~/Desktop/vit-test-project
```

Expected:

```
  Initialized vit project in /Users/.../vit-test-project
  Created: .vit/, timeline/, assets/
...
```bash
cd ~/Desktop/vit-test-project
git remote add origin git@github.com:yourteam/vit-test-project.git
...
```bash
git clone git@github.com:yourteam/vit-test-project.git ~/Desktop/vit-test-project
```

### 1e. (Optional) Set project dir to skip the folder picker

```bash
export VIT_PROJECT_DIR=~/Desktop/vit-test-project
```
...
2. Create a new project (e.g. "Vit Test Project")
...
Go to **Workspace > Scripts**. You should see all 7 Vit items. If not, restart Resolve.

### Step 3 — Save the initial version

1. **Workspace > Scripts > Vit - Save Version**
2. First time: a folder picker dialog appears — select `~/Desktop/vit-test-project`
...
cd ~/Desktop/vit-test-project
vit log
# → abc1234 (HEAD -> main) vit: initial rough cut with 2 clips
# → def5678 vit: initial snapshot
...
1. **Workspace > Scripts > Vit - New Branch**
2. Enter: `color-grade`
3. Click OK
...
2. **Workspace > Scripts > Vit - Save Version**
3. Message: `trimmed clip B`

### Step 8 — Merge the branches

1. **Workspace > Scripts > Vit - Merge Branch**
...
1. **Workspace > Scripts > Vit - Status**
...
Both machines need:
- vit installed in `.venv`
- `vit install-resolve` run once
...
2. **Vit - Save Version** → message: `rough cut v1`
3. **Vit - Push** → pushes `main` to GitHub

**Person B (Colorist) — gets the rough cut:**

4. **Vit - Pull & Restore** → pulls `main` from GitHub, timeline loads with the rough cut
5. **Vit - New Branch** → creates `color-grade`
6. Go to the Color page, grade the clips
7. **Vit - Save Version** → message: `initial color pass`
8. **Vit - Push** → pushes `color-grade` to GitHub

**Person A (Editor) — continues editing while B was grading:**

9. **Vit - New Branch** → creates `fine-cut`
10. Trim clips, rearrange, add b-roll
11. **Vit - Save Version** → message: `fine cut with b-roll`
12. **Vit - Switch Branch** → back to `main`
13. **Vit - Pull & Restore** → gets Person B's `color-grade` branch
14. **Vit - Merge Branch** → merges `color-grade` into `main`
15. **Vit - Merge Branch** → merges `fine-cut` into `main`
...
16. **Vit - Push** → pushes the merged `main` to GitHub

**Person B (Colorist) — gets the merged result:**

17. **Vit - Switch Branch** → back to `main`
18. **Vit - Pull & Restore** → gets the merged timeline with both their grading and A's fine cut
...
If `GEMINI_API_KEY` is set, vit offers AI-assisted resolution: the LLM reads both versions, sees the orphan, and removes the stale color grade. The user confirms before it's applied.
...
# Create test project
mkdir /tmp/vit-cli-test
vit init /tmp/vit-cli-test
cd /tmp/vit-cli-test
...
"
vit commit -m "added test clip"

# Branch off for color work
vit branch color-work
...
"
vit commit -m "color grade on branch"
...
- Run `vit install-resolve` from terminal
...
### "Not a vit project" or wrong project opens

- Delete `~/.vit/last_project` to reset the saved project path
- Set `VIT_PROJECT_DIR` env var before launching Resolve
...
Vit only versions **timeline metadata** (JSON), not media files. All collaborators need the same media files at the same paths on their machines. Use a shared drive, Dropbox, or manually copy the files. The `assets/manifest.json` lists all referenced media paths.
...
If two people edit the same domain (e.g., both change `cuts.json`), git may produce a conflict. Use `vit merge <branch>` from terminal for AI-assisted resolution:

```bash
export GEMINI_API_KEY=your-key-here
vit merge problematic-branch
```

### Color changes show "Nothing to commit"

Vit captures color grades by exporting DRX stills from Resolve's Gallery. If grade export fails:
...
2. Then run **Vit - Switch Branch** again
